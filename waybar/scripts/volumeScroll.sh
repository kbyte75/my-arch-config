...
update_volume() {
    ...
    if [[ "$current_volume" -eq 0 ]]; then
        notify-send "The volume is muted." -i ~/.config/waybar/icons/volume_mute.svg -t 1000
    elif [[ "$current_volume" -eq 100 ]]; then
        notify-send "Maximum volume reached $current_volume%" -i ~/.config/waybar/icons/volume_max.svg -t 1000
    else
        # Send the notification with the action text and the corresponding icon
        notify-send "$notify_text" -i ~/.config/waybar/icons/volume_up.svg -t 1000
    fi
}
