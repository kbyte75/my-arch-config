// Config created by Keyitdev https://github.com/Keyitdev/one-piece
...
RowLayout {
    ...
    Repeater {
        ...
        RoundButton {
            ...
            icon.height: 2 * Math.round((root.font.pointSize * 2) / 2)
            icon.width: 2 * Math.round((root.font.pointSize * 2) / 2)
            icon.color: config.SystemButtonsIconsColor
            palette.buttonText: config.SystemButtonsIconsColor
            display: AbstractButton.TextUnderIcon
            visible: true //config.HideSystemButtons != "true" && (config.BypassSystemButtonsChecks == "true" ? 1 : modelData[2])
            hoverEnabled: true
